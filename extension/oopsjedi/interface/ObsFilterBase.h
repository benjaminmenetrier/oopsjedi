...
#ifndef OOPSJEDI_INTERFACE_OBSFILTERBASE_H_
#define OOPSJEDI_INTERFACE_OBSFILTERBASE_H_

#include <memory>
#include <string>

#include "oopsjedi/base/ObsVector.h"
#include "oopsjedi/generic/ObsFilterBase.h"
#include "oopsjedi/interface/GeoVaLs.h"
#include "oopsjedi/interface/ObsDataVector.h"
...
/// interface::ObsFilterBase overrides oopsjedi::ObsFilterBase methods to pass OBS-specific
...
class ObsFilterBase : public oopsjedi::ObsFilterBase<OBS> {
 ...
 public:
  // Overrides of oopsjedi::ObsFilterBase methods, converting between oops interface classes and
  ...
  void postFilter(const GeoVaLs<OBS> & gv,
                  const oopsjedi::ObsVector<OBS> &ov,
                  const oopsjedi::ObsVector<OBS> &bv,
  ...
  virtual void checkFilterData(const oopsjedi::FilterStage filterStage) = 0;
};
...
class FilterMaker : public FilterFactory<OBS> {
 ...
 public:
  ...
  std::unique_ptr<oopsjedi::ObsFilterBase<OBS>> make(const ObsSpace_ & os,
...
#endif  // OOPSJEDI_INTERFACE_OBSFILTERBASE_H_
