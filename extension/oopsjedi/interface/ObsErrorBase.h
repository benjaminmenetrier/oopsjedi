...
#ifndef OOPSJEDI_INTERFACE_OBSERRORBASE_H_
#define OOPSJEDI_INTERFACE_OBSERRORBASE_H_

#include <memory>
#include <string>

#include "oopsjedi/base/ObsVector.h"
#include "oopsjedi/generic/ObsErrorBase.h"
...
///
/// interface::ObsErrorBase overrides oopsjedi::ObsErrorBase methods to pass OBS-specific
...
class ObsErrorBase : public oopsjedi::ObsErrorBase<OBS> {
 ...
 public:
  ...
  // Overrides of oopsjedi::ObsErrorBase methods, converting between oops interface classes and
  // OBS-specific classes operated upon by OBS-specific implementations of the ObsError interface.

  void multiply(oopsjedi::ObsVector<OBS> &dy) const final {
    this->multiply(dy.obsvector());
  }

  void inverseMultiply(oopsjedi::ObsVector<OBS> &dy) const final {
    this->inverseMultiply(dy.obsvector());
  }

  void randomize(oopsjedi::ObsVector<OBS> &dy) const final {
    this->randomize(dy.obsvector());
  }

  oopsjedi::ObsVector<OBS> obserrors() const final {
    return oopsjedi::ObsVector<OBS>(this->getObsErrors(), timeComm_);
  }

  void update(const oopsjedi::ObsVector<OBS> &dy) final {
    this->update(dy.obsvector());
  }

  oopsjedi::ObsVector<OBS> inverseVariance() const final {
    return oopsjedi::ObsVector<OBS>(this->getInverseVariance(), timeComm_);
  }
...
class ObsErrorMaker : public ObsErrorFactory<OBS> {
 public:
  typedef oopsjedi::ObsErrorBase<OBS>    ObsErrorBase_;
  typedef oopsjedi::ObsErrorFactory<OBS> ObsErrorFactory_;
  typedef oopsjedi::ObsSpace<OBS>        ObsSpace_;
...
#endif  // OOPSJEDI_INTERFACE_OBSERRORBASE_H_
