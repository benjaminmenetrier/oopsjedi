...
#ifndef OOPSJEDI_INTERFACE_INCREMENT_H_
#define OOPSJEDI_INTERFACE_INCREMENT_H_
...
#include "oopsjedi/base/GeneralizedDepartures.h"
...
class Increment : public oopsjedi::GeneralizedDepartures,
 ...
 public:
  ...
  static const std::string classname() {return "oopsjedi::Increment";}
 ...
 public:
  ...
  const utiljedi::DateTime validTime() const {return increment_->validTime();}
  ...
  void zero(const utiljedi::DateTime & date);
 ...
 public:
  ...
  LocalIncrement getLocal(const GeometryIterator_ & iter) const
    {ABORT("not implemented in oopsjedi"); return LocalIncrement();}
...
void Increment<MODEL>::diff(const State_ & x1, const State_ & x2) {
  ...
  utiljedi::Timer timer(classname(), "diff");
  fset_.clear();
...
template<typename MODEL>
void Increment<MODEL>::zero(const utiljedi::DateTime & tt) {
  Log::trace() << "Increment<MODEL>::zero starting" << std::endl;
  utiljedi::Timer timer(classname(), "zero");
  fset_.clear();
  increment_->zero(tt);
...
void Increment<MODEL>::axpy(const double & zz, const Increment & dx, const bool check) {
  ...
  utiljedi::Timer timer(classname(), "axpy");
  fset_.clear();
...
void Increment<MODEL>::schur_product_with(const Increment & dx) {
  ...
  utiljedi::Timer timer(classname(), "schur_product_with");
  fset_.clear();
...
#endif  // OOPSJEDI_INTERFACE_INCREMENT_H_
