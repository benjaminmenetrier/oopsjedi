...
#ifndef OOPSJEDI_INTERFACE_SAMPLEDLOCATIONS_H_
#define OOPSJEDI_INTERFACE_SAMPLEDLOCATIONS_H_
...
class SampledLocations : public utiljedi::Printable,
                         private utiljedi::ObjectCounter<SampledLocations<OBS>> {
  typedef typename OBS::SampledLocations SampledLocations_;
 public:
  static const std::string classname() { return "oopsjedi::SampledLocations"; }
  ...
  /// Interpolation times.
  const std::vector<utiljedi::DateTime> & times() const;
...
SampledLocations<OBS>::SampledLocations(const eckit::Configuration & conf,
  ...
  utiljedi::Timer timer(classname(), "SampledLocations");
...
const std::vector<double> & SampledLocations<OBS>::latitudes() const {
  ...
  utiljedi::Timer timer(classname(), "latitudes");
...
const std::vector<double> & SampledLocations<OBS>::longitudes() const {
  ...
  utiljedi::Timer timer(classname(), "longitudes");
...
template <typename OBS>
const std::vector<utiljedi::DateTime> & SampledLocations<OBS>::times() const {
  Log::trace() << "SampledLocations<OBS>::times starting" << std::endl;
  utiljedi::Timer timer(classname(), "times");
...
#endif  // OOPSJEDI_INTERFACE_SAMPLEDLOCATIONS_H_
