...
#include "oopsjedi/util/LibOOPS.h"
...
LibOOPS::LibOOPS() : Library("oopsjedi"), rank_(0), test_(false),
                     debug_(false), predebug_("OOPSJEDI_DEBUG"),
                     trace_(false), pretrace_("OOPSJEDI_TRACE") {
}
 ...
 * To be called in `main()` by constructor of `oopsjedi::Run`.  This method initializes MPI and
...
void LibOOPS::initialise() {
  ...
  rank_ = oopsjedi::mpi::world().rank();

  const int it = getEnv("OOPSJEDI_TRACE", 0);
  if (it > 0 && rank_ == 0) trace_ = true;
  if (it < 0) {
    trace_ = true;
    pretrace_ += "[" + std::to_string(rank_) + "]";
  }
  const int id = getEnv("OOPSJEDI_DEBUG", 0);
  if (id > 0 && rank_ == 0) debug_ = true;
  if (id < 0) {
    debug_ = true;
    predebug_ += "[" + std::to_string(rank_) + "]";
  }
  const int do_trapfpe = getEnv("OOPSJEDI_TRAPFPE", 0);
  int do_abortfpe;
  if (do_trapfpe) {
    ...
    do_abortfpe = getEnv("OOPSJEDI_ABORTFPE", 1);
    trap_sigfpe(do_abortfpe);
  }

#ifdef ENABLE_GPTL
  do_profile = getEnv("OOPSJEDI_PROFILE", 0);
#endif
}

/** Add a rank-dependent tee file
 * To be called in `main()` by constructor of `oopsjedi::Run` when the outputfile argument is specified
 */
 ...
 * To be called in on leaving `main()` by the destructor of `oopsjedi::Run`.
 */
void LibOOPS::finalise(bool finaliseMPI) {
    ...
    if (do_profile) {
      int ret;
      if (oopsjedi::mpi::world().rank() == 0) {
        oopsjedi::Log::info() << "Calling GPTLpr(0)" << std::endl;
        ret = GPTLpr(0);                       // Print timing info for rank 0
      }
      // Summarize timing info across ranksL For now assume MPI_COMM_WORLD
      if (oopsjedi::mpi::world().size() > 1) {
        oopsjedi::Log::info() << "Calling GPTLpr_summary" << std::endl;
...
eckit::Channel& LibOOPS::statsChannel() const {
  ...
  if (rank_ == 0) {
    statsChannel_.reset(new eckit::Channel(
      new eckit::PrefixTarget("OOPSJEDI_STATS", new eckit::OStreamTarget(eckit::Log::info()))));
  } else {
    statsChannel_.reset(new eckit::Channel());
  }
