...
#ifndef OOPSJEDI_BASE_INCREMENT_H_
#define OOPSJEDI_BASE_INCREMENT_H_
...
#include "oopsjedi/util/gatherPrint.h"
...
class Increment : public interface::Increment<MODEL> {
 ...
 public:
  ...
  Increment(const Geometry_ & geometry, const Variables & variables, const utiljedi::DateTime & date);
  ...
  /// Constructor from an oops::State<MODEL> and the corresponding oopsjedi::Geometry<MODEL>
  Increment(const Increment_ &, const Geometry_ &);
  ...
  void shift_forward(const utiljedi::DateTime & dt);
  /// Shift backward in time by \p dt
  void shift_backward(const utiljedi::DateTime & dt);
...
template<typename MODEL>
Increment<MODEL>::Increment(const Increment_ & increment, const Geometry_ & resol) :
  interface::Increment<MODEL>(increment), resol_(resol), timeComm_(&resol.timeComm())
{}
...
template<typename MODEL>
void Increment<MODEL>::shift_forward(const utiljedi::DateTime & begin) {
...
// Send values of M.dx_i at end of my subwindow to next subwindow
  if (mytime + 1 < timeComm_->size()) {
    oopsjedi::mpi::send(*timeComm_, *this, mytime+1, tag);
  }

// Receive values at beginning of my subwindow from previous subwindow
  if (mytime > 0) {
    oopsjedi::mpi::receive(*timeComm_, *this, mytime-1, tag);
  } else {
    this->zero(begin);
  }
...
template<typename MODEL>
void Increment<MODEL>::shift_backward(const utiljedi::DateTime & end) {
...
void Increment<MODEL>::print(std::ostream & os) const {
  if (timeComm_->size() > 1) {
    utiljedi::gatherPrint(os, this->increment(), *timeComm_);
  } else {
    os << this->increment();
  }
}
...
State<MODEL> & operator+=(State<MODEL> & xx, const Increment<MODEL> & dx) {
  ...
  utiljedi::Timer timer("oopsjedi::Increment", "operator+=(State, Increment)");
...
#endif  // OOPSJEDI_BASE_INCREMENT_H_
