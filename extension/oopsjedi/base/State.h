...
#ifndef OOPSJEDI_BASE_STATE_H_
#define OOPSJEDI_BASE_STATE_H_

#include <memory>
#include <string>
#include <vector>

#include "oopsjedi/base/StateParametersND.h"
...
class State : public interface::State<MODEL> {
 ...
 public:
  ...
  State(const State_ &, const Geometry_ &);
...
template<typename MODEL>
State<MODEL>::State(const State_ & state, const Geometry_ & resol) :
  interface::State<MODEL>(state), resol_(resol), commTime_(&resol.timeComm())
{}
    ...
    if (type == "fc" || type == "ens") {
      if (!conf.has("date"))
        throw eckit::BadValue("'date' was not set in the parameters passed to write() "
                              "even though 'type' was set to '" + type + "'", Here());
      const utiljedi::DateTime antime(conf.getString("date"));
      if (dateCols) {
        prefix += "." + antime.toString();
      } else {
        prefix += "." + antime.toStringIO();
      }
      const utiljedi::Duration step = this->validTime() - antime;
...
void State<MODEL>::print(std::ostream & os) const {
  if (commTime_->size() > 1) {
    utiljedi::gatherPrint(os, this->state(), *commTime_);
  } else {
    os << this->state();
  }
}
